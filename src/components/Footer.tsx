    ...
    <footer className="bg-background border-t border-border">
      <div className="container mx-auto px-4 py-12">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          ...
          <div>
            <div className="flex items-center gap-3 mb-4">
              ...
              <h3 className="text-2xl font-bold text-foreground">MindSpace</h3>
            </div>
            ...
            <div className="space-y-3">
              {quickLinks.map((link, index) => (
                <Button
                  key={index}
                  variant="ghost"
                  className="w-full justify-start text-muted-foreground hover:text-foreground hover:bg-muted p-0 h-auto"
                >
          ...
          {/* Crisis Support */}
          <div>
            <h4 className="text-lg font-semibold mb-6 text-foreground">Crisis Support</h4>
            <div className="space-y-4">
              {crisisResources.map((resource, index) => (
                <div key={index} className="bg-muted/50 rounded-lg p-4 border border-border">
                  <h5 className="font-medium text-foreground mb-1 text-sm">{resource.name}</h5>
                  <p className="text-muted-foreground text-sm mb-1">{resource.contact}</p>
                  <p className="text-muted-foreground/80 text-xs">{resource.description}</p>
                </div>
              ))}
            </div>
          </div>
        ...
        <div className="border-t border-border mt-12 pt-8">
          <div className="bg-amber-50/50 dark:bg-amber-900/10 border border-amber-200/50 dark:border-amber-800/50 rounded-lg p-6 mb-8">
